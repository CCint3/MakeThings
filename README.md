...
- `(x ^ 1) + 2 * (x & 1)`转化为`x + 1`：
  ...
  推导1：若`x&1`等于1时，`(x ^ 1)`结果转化为`x-1`；`2 * (x & 1)`结果转化为`2`；`x-1+2`转化为`x+1`；

  推导2：若`x&1`等于0时，`(x ^ 1)`结果转化为`x+1`；`2 * (x & 1)`结果转化为`0`；`x+1+0`转化为`x+1`；

- `(x ^ 2) + 2 * (x & 2)`转化为`x + 2`：

  推导1：若`x&2`等于2时，`(x ^ 2)`结果转化为`x-2`；`2 * (x & 2)`结果转化为`4`；`x-2+4`转化为`x+2`；

  推导2：若`x&2`等于0时，`(x ^ 2)`结果转化为`x+2`；`2 * (x & 2)`结果转化为`0`；`x+2+0`转化为`x+2`；

根据以上两个式子可以得出`(x ^ y) + 2 * (x & y)`可以转化为`x + y`
